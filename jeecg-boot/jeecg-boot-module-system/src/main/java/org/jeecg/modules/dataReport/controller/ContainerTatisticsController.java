...
public class ContainerTatisticsController {
    ...
    public Result<IPage<ContainerTatistics>> queryPageList(ContainerTatistics containerTatistics,
        ...
        if (oConvertUtils.isNotEmpty(bolcode)) {
            queryWrapper.like("t1.bl_no", bolcode);
        }
        //箱号
    ...
    public ModelAndView exportXls(ContainerTatistics containerTatistics, HttpServletRequest request, HttpServletResponse response) {
        // Step.1 组装查询条件
        QueryWrapper<ContainerTatistics> queryWrapper = QueryGenerator.initQueryWrapper(containerTatistics, request.getParameterMap());
        String bolcode = request.getParameter("bolcodeQuery");
        if (oConvertUtils.isNotEmpty(bolcode)) {
            queryWrapper.like("t1.bl_no", bolcode);
        }
        //箱号
        String containerNo = request.getParameter("containerNoQuery");
        if (oConvertUtils.isNotEmpty(containerNo)) {
            queryWrapper.like("t.container_no", containerNo);
        }
        ...
        List<ContainerTatistics> pageList = containerTatisticsService.getContainerTatistics(queryWrapper);
        //导出文件名称
        mv.addObject(NormalExcelConstants.FILE_NAME, "集装箱业务统计表列表");
        mv.addObject(NormalExcelConstants.CLASS, ContainerTatistics.class);
        mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("集装箱业务统计表列表数据", "导出人:" + user.getRealname() ,"导出信息"));
